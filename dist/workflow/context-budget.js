...
exports.computeBudgets = computeBudgets;
exports.truncateAtSectionBoundary = truncateAtSectionBoundary;
exports.assembleContext = assembleContext;
exports.capPreamble = capPreamble;
// ─── Budget Constants ─────────────────────────────────────────────────────────
 ...
 */
function computeBudgets(contextWindowTokens) {
 ...
 */
function truncateAtSectionBoundary(content, maxChars) {
 ...
 */
function assembleContext(blocks, contextWindowTokens) {
 ...
 */
function capPreamble(preamble, contextWindowTokens) {
    const budget = computeBudgets(contextWindowTokens);
    const cap = Math.min(MAX_PREAMBLE_CHARS, budget.systemBudgetChars);
    if (preamble.length <= cap)
        return preamble;
