...
exports.scanBuildContext = scanBuildContext;
exports.formatBuildContextReport = formatBuildContextReport;
exports.formatBuildContextJSON = formatBuildContextJSON;
...
function scanBuildContext() {
    ...
    const activeSessions = (0, git_hooks_1.getActiveSessions)().map(s => ({
        actor: s.actor,
        branch: s.branch,
        itemId: s.itemId,
 ...
 */
function scanReadyBranches(root, warnings) {
    const readyBranches = [];
    // Get all branches (local and remote tracking)
    let allBranches = [];
    try {
        const output = (0, child_process_1.execSync)('git branch -a --format="%(refname:short)"', { encoding: 'utf-8' });
        allBranches = output.trim().split('\n').filter(b => b.startsWith('feature/') || b.startsWith('fix/') || b.startsWith('chore/') ||
    ...
    try {
        const backlog = (0, db_1.listBacklog)(db);
        for (const branch of allBranches) {
            ...
            let filesChanged = [];
            try {
                lastCommit = (0, child_process_1.execSync)(`git log -1 --format="%H" ${branch} 2>/dev/null`, { encoding: 'utf-8' }).trim();
                lastCommitDate = (0, child_process_1.execSync)(`git log -1 --format="%ci" ${branch} 2>/dev/null`, { encoding: 'utf-8' }).trim();
                const filesOutput = (0, child_process_1.execSync)(`git diff --name-only main...${branch} 2>/dev/null || git diff --name-only HEAD...${branch} 2>/dev/null`, { encoding: 'utf-8' });
            ...
            let prUrl;
            try {
                const prOutput = (0, child_process_1.execSync)(`gh pr list --head "${cleanBranch}" --json url,state --limit 1 2>/dev/null`, { encoding: 'utf-8' });
 ...
 */
function extractItemIdFromBranch(branch) {
    const patterns = [
 ...
 */
function scanADRs(root) {
    const db = (0, db_1.getDatabase)();
    try {
        const accepted = (0, db_1.listADRs)(db, 'accepted').map(adr => ({
            id: adr.id,
            title: adr.title,
            status: adr.status,
        ...
        const proposed = (0, db_1.listADRs)(db, 'proposed').map(adr => ({
            id: adr.id,
            title: adr.title,
            status: adr.status,
            decision: adr.decision || '',
 ...
 */
function extractModulesFromADR(context) {
 ...
 */
function computeDependencies(readyBranches, root) {
    const dependencies = [];
 ...
 */
function computeMergeOrder(readyBranches, dependencies) {
 ...
 */
function scanBacklogState(root) {
    const db = (0, db_1.getDatabase)();
    try {
        const all = (0, db_1.listBacklog)(db);
        return {
 ...
 */
function formatBuildContextReport(report) {
    const lines = [];
    lines.push('╔══════════════════════════════════════════════════════════════╗');
 ...
 */
function formatBuildContextJSON(report) {
