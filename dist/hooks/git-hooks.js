...
exports.deregisterSession = deregisterSession;
 ...
 */
function registerSession(actor, branch, itemId, actorType) {
 ...
 */
function getActiveSessions(excludeActor) {
    const root = (0, db_1.findProjectRoot)();
    ...
    try {
        ...
        if (excludeActor)
            sessions = sessions.filter(s => s.actor !== excludeActor);
 ...
 */
function deregisterSession(actor) {
    const root = (0, db_1.findProjectRoot)();
    const sessionsFile = path.join(root, '.nexus', 'active-sessions.json');
    if (!fs.existsSync(sessionsFile))
        return;
